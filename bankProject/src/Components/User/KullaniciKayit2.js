...
export default class KullaniciKayit2 extends React.Component {
  constructor(props) {
	...
	componentDidMount = () => {
    	...
    	this.setState({
    		maxDate: new Date(year-18, nextMonth, day, hours, min, sec, millisec),
    	})
	}
  KayitOl = () => {
      ...
      fetch("http://bankrestapi.azurewebsites.net/api/Musteri/Add", {
        ...
        .then(() => {
          Alert.alert("Başarılı!",
            "Kayıt işlemi başarılı bir şekilde tamamlanmıştır.", [{
              text: 'OK', onPress: () => {
  ...
  handlePicker = (datetime) => {
    let musteri = this.state.Musteri;
    musteri.dogumTarihi = moment(datetime).format();
    this.setState({
      isDateTimePickerVisible: false,
      Musteri: musteri,
