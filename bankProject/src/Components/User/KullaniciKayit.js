...
export default class KullaniciKayit extends React.Component {
  ...
  KayitOl = () => {
    fetch("http://bankrestapi.azurewebsites.net/api/Musteri/GetMusteriByTckn?tckn=" + this.state.Musteri.tckn)
      .then(res => res.json())
      .then(response => {
        if (response == null) {
          let Musteri = this.state.Musteri;
          ...
          Alert.alert("Kayıtlı kullanıcı!", "Zaten böyle bir kullanıcı bulunmaktadır!")
        }
      })
  }
  render() {
